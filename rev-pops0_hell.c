...
/**
  * find_path - finds if a cmd exists in a path and returns the path
  * @path: paths to look for
  * @command: command to search for path
  * Return: path where command is returning
  */
...
/**
 * get_env - get current env as a malloc'd, NULL-terminating char**
...
/**
 * get_full_command - get the full cmd with the orinal path prepared.
 * @path: possible paths are included
 * @command: the base command
 * Return: the original path + command (leave command alone if already done)
 */
